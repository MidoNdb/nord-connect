export default function Home() {
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 animate-fade-in">
      ...
      <section className="relative bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="relative px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
          <div className="max-w-4xl">
            ...
            {/* Description */}
            <p className="text-xs sm:text-sm lg:text-base text-gray-600 dark:text-gray-400 leading-relaxed mb-4 sm:mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 hover:from-blue-700 hover:via-purple-700 hover:to-pink-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2 text-sm">
                Démarrer
              ...
              <button className="bg-white dark:bg-slate-800 text-gray-900 dark:text-white px-6 py-3 rounded-xl font-bold border-2 border-gray-200 dark:border-slate-700 transition-all flex items-center justify-center gap-2 text-sm">
                En savoir plus
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Stats - RESPONSIVE PARFAIT */}
      <section className="grid grid-cols-2 gap-3 sm:gap-4">
        <StatCardMobile number="1,285" label="Établissements" color="blue" trend="+12%" />
        <StatCardMobile number="85,420" label="Ordinateurs" color="green" trend="+8%" />
        <StatCardMobile number="12.4M€" label="Économies" color="purple" trend="+15%" />
        <StatCardMobile number="1,230t" label="CO₂ évités" color="pink" trend="+10%" />
      </section>

      {/* Features */}
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <FeatureCardMobile title="Forum" description="Échangez avec la communauté" color="blue" />
        <FeatureCardMobile title="Carte" description="Trouvez des mentors près de vous" color="purple" />
        <FeatureCardMobile title="Témoignages" description="Histoires inspirantes" color="pink" />
      </section>

      {/* CTA Final */}
      <section className="relative bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="relative px-4 py-8 sm:py-12 text-center">
          ...
          <p className="text-blue-100 text-xs sm:text-sm mb-6 max-w-lg mx-auto">
            Plus de 1000 établissements ont déjà fait le choix du numérique responsable.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-md mx-auto">
            <button className="bg-white hover:bg-gray-50 text-blue-600 px-6 py-3 rounded-xl font-bold shadow-xl transition-all text-sm">
              Créer mon compte
            </button>
            <button className="bg-white/10 backdrop-blur-md text-white px-6 py-3 rounded-xl font-bold border-2 border-white/20 transition-all text-sm">
              Contacter
            </button>
          </div>
        </div>
      ...
      {/* Avantages */}
      <section className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 p-4 sm:p-6">
        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-4 text-center">
          Pourquoi NIRD Connect ?
        </h2>
        
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <BenefitCardMobile title="Économies 40-70%" description="Réduisez vos coûts" />
          <BenefitCardMobile title="Impact environnemental" description="Moins de CO₂" />
          <BenefitCardMobile title="Communauté active" description="Centaines d'établissements" />
          <BenefitCardMobile title="Indépendance tech" description="Contrôle infrastructure" />
          <BenefitCardMobile title="Formation continue" description="Ressources pédago" />
          <BenefitCardMobile title="Support technique" description="Entraide partagée" />
        </div>
      </section>
    </div>
  );
...
function StatCardMobile({ number, label, color, trend }) {
  const colors = {
  ...
  return (
    <div className={`${bgColors[color]} rounded-xl p-3 border border-gray-200 dark:border-slate-700`}>
      {/* Header avec nombre + badge */}
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className={`text-lg sm:text-xl lg:text-2xl font-bold bg-gradient-to-r ${colors[color]} bg-clip-text text-transparent leading-tight`}>
          {number}
        </div>
        <span className="text-xs font-bold text-green-600 bg-green-100 px-1.5 py-0.5 rounded-full shrink-0">
          {trend}
        </span>
      </div>
...
function FeatureCardMobile({ title, description, color }) {
  const colors = {
    blue: 'from-blue-500 to-blue-600',
    purple: 'from-purple-500 to-purple-600',
    pink: 'from-pink-500 to-pink-600'
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border-2 border-gray-200 dark:border-slate-800">
      <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${colors[color]} mb-3`}></div>
      ...
      <button className={`text-xs font-semibold bg-gradient-to-r ${colors[color]} bg-clip-text text-transparent flex items-center gap-1`}>
        Découvrir
...
function BenefitCardMobile({ title, description }) {
  return (
    <div className="flex gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
      ...
      <div className="min-w-0">
        <h4 className="font-bold text-gray-900 dark:text-white mb-0.5 text-xs sm:text-sm">
          {title}
        </h4>
