...
export default function App() {
  ...
  // ⬇️⬇️⬇️ Fonction corrigée — maintenant elle voit setActiveFeature
  const renderFeature = () => {
    switch (activeFeature) {
      case 'home':
        return <Home onStartGame={() => setActiveFeature('game')} />;
      case 'forum':
        return <Forum />;
      case 'map':
        return <MapPage />;
      case 'testimonials':
        return <Testimonials />;
      case 'game':
        return <Game onBackToHome={() => setActiveFeature("home")} />;
      default:
        return <Home onStartGame={() => setActiveFeature('game')} />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
      ...
      <div className={`transition-all duration-300 ${sidebarCollapsed ? 'ml-20' : 'ml-72'}`}>
        ...
        <header className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 sticky top-0 z-40 backdrop-blur-sm">
...
function getPageSubtitle(feature) {
  return {
    ...
    testimonials: 'Témoignages d’écoles engagées',
